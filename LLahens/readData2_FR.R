  ...
  if (checkIndices(indices)[["check"]])
  {
    ...
    for (i in 1:nbCompound)
    {
      data = fread(filepath, skip = indices[i, startLine - 2], nrows = n,
                   select = c("Sample Text", "Type", "ng/L", "%Dev","Conc. Dev. Flagged", "Sig/Noise Flag", "CD Flag"))
      
      names(data) = rename(df = data,
                           colNamesToChange = c("Sample Text", "ng/L", "%Dev", "Conc. Dev. Flagged", "Sig/Noise Flag", "CD Flag"),
                           newColNames = c("sampleText", "ppb", "percentDev", "concentrationFlag", "noiseFlag", "cdFlag"))
      ...
      data[!is.na(group),sampleInGroup := .N, by = group] # removed !is.na(group)
      
      data[!is.na(group),c("mean_ppb", "sd_ppb") := .(mean(ppb, na.rm = TRUE), sd(ppb, na.rm = TRUE)), by = group] # removed !is.na(group)
      data[!is.na(group),nonNA_sample := sum(!is.na(ppb)), by = group]# removed 
      
      # add %DEv
      #data[ , percentDev := ifelse(concentrationFlag=="YES",percentDev,NA)]
