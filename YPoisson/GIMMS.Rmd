...
Here I will present the method used to extract phenological variables from GIMMS and MODIS NDVI data. This document is a bit detailed in order to show exactly what I have done, so it can be modified and critiqued if needed. 
...
Although the fit is more flexible and better adapts to the time series, one of the drawback is that it is more difficult to quantify identify specific time points such as the onset of spring. The maximal slope of increase in NDVI values can also show up anywhere during green-up whihc may make the the dates obtained more variables.
...
Once the time-series object is made, put it all in an interactive dygraph.
...
Now, suppose we have extracted values for each pixel and each variable and stored them stored them in an R session and a list `lr` of rasters.
...
Now, let's say we have polygons and we want to extract and summarize phenological variables for them. We need to first extract values and then store them in the `data` slot of the regions `SpatialPolygonsDataFrame`.
...
There are a couple differences with the MODIS data. First, with each pixel value there is a julian date given on which the measurements was taken, gining a bit more temporal resolution compared to GIMMS data. Also, there are two "offset" satellites providing a value every 16 days. When added, the two satellites provide a value about every 8 days. Thus, a couple adjustement were made to use the julian dates and to accomodate the mote numerous data. 
