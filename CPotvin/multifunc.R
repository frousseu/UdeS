...
ld<-lapply(ld,function(j){
  ...
  coly<-alpha(c("darkred","darkblue","darkgreen"),1)
  mgp<-c(3,0.25,0)
  tcl<--0.2
  ...
  invisible(ans<-lapply(seq_along(ld),function(k){
    ...
    rec<-c(par("usr")[2]+0.05,par("usr")[3],par("usr")[2]+0.17,par("usr")[4])
    ...
    labs<-at[1:length(at)]
    text(x=rec[3],y=seq(rec[2],rec[4],l=length(at)),cex=0.8,labels=round(labs,2),adj=c(-0.2,0.5),font=0,xpd=TRUE)
...
title(ylab="Slope of diversity",line=1.5,cex.lab=1.2)
...
lapply(seq_along(ans2),function(i){
  ...
  lines(x$th,x$"2.5 %",col=alpha(coly[i],0.15),lwd=2,lty=1)
  lines(x$th,x$"97.5 %",col=alpha(coly[i],0.15),lwd=2,lty=1)
...
th<-getFuncsMaxed(germany, vars, threshmin=0.05, threshmax=0.99, prepend=c("plot","Diversity"), maxN=7)

slopes<-getCoefTab(funcMaxed ~ Diversity, data=th, coefVar="Diversity", family=quasipoisson(link="identity"))


plot(0,0,ylim=c(0,5),xlim=range(th$Diversity),type="n")
thr<-sort(unique(th$thresholds))
l<-split(th,th$thresholds)
invisible(lapply(seq_along(l),function(j){
  i<-l[[j]]
  points(jitter(i$Diversity),jitter(i$funcMaxed)) 
  m<-lm(funcMaxed~Diversity,data=i)
  abline(m,col=colo.scale(1:length(l),c("blue","pink","red"))[j],lwd=2)
}))
